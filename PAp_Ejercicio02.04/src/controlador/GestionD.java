...
public class GestionD {
	...
	private List<Atleta> getAtletasArchivo() throws IOException {
		...
		RandomAccessFile out = new RandomAccessFile(pathD, "rw");
		try {
			while (true) {
				Atleta a = new Atleta();
				Competencia c = new Competencia();
				Resultado r = new Resultado();
				a.setNombre(out.readUTF());
				a.setApellido(out.readUTF());
				a.setCedula(out.readUTF());
				a.setCodigo(out.readUTF());
				r.settInicial(out.readUTF());
				r.settFinal(out.readUTF());
				c.setNombre(out.readUTF());
				c.setCategoria(out.readUTF());
				a.setResultado(r);
				a.setCompetencia(c);
				atletas.add(a);
			}
